...
function getPlugins() {
    ...
    fs.readdirSync('src/views/').filter(m => m !== 'fragment').forEach(moduleName => {
        ...
        fs.readdirSync(`src/views/${moduleName}`).forEach(page => {
            ...
            const isE = fs.existsSync(`src/static/js/${moduleName}/${page.replace('.html', '')}/main.js`);
            if (isE) {
                entries[chunk] = path.resolve(__dirname, `../src/static/js/${moduleName}/${page.replace('.html', '')}/main.js`);
            }
